...
public class AGeneticoP1F4 extends AGenetico {
	
	private int n;
	
	public AGeneticoP1F4(int poblacion, int generaciones, double porcCruces, double porcMutacion, double tolerancia, boolean elitismo, int tipoSel, int n)
	{
	...
	private void seleccionEstocastico() {
		...
		Cromosoma[] nuevaPob = new CromosomaP1F4[tamPob];
		for(int i = 0; i < tamPob; i++){
	...
	private void seleccionTorneo(int tipo){
		Cromosoma subpoblacion[] = new CromosomaP1F4[2]; //Conjunto a valorar
		Cromosoma poblacionAux[] = new CromosomaP1F4[tamPob]; //Nueva generación
	...
	public void reproduccion() {
		...
		int puntoCruce;
		double prob;
		Cromosoma hijo1 = new CromosomaP1F4(tolerancia, n);
		Cromosoma hijo2 = new CromosomaP1F4(tolerancia, n);
		Random rnd = new Random();
	...
	private void cruce(Cromosoma padre1, Cromosoma padre2, Cromosoma hijo1, Cromosoma hijo2, int puntoCruce)
	{
		...
		// Primera fase
		while(i < CromosomaP1F4.N_GENES && k < puntoCruce)
		{
